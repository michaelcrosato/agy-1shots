...
async function runEmpiricalTests() {
  ...
  try {
    ...
    if (res404.status !== 404)
      throw new Error("Expected 404 for non-existent directory");
    ...
    if (resEmpty.status !== 200)
      throw new Error(`Expected 200, got ${resEmpty.status}`);
    ...
    const hasConfigSuggestion = dataEmpty.suggestions.some(
      (s) =>
        ...
        s.description.includes("Missing package.json"),
    );
    const hasImplSuggestion = dataEmpty.suggestions.some(
      (s) =>
        s.type === "implementation" &&
        s.description.includes("Missing main execution script"),
    );
    if (!hasConfigSuggestion || !hasImplSuggestion) {
      throw new Error(
        "Missing package.json or index.js suggestion was not returned dynamically",
      );
    }
    console.log(
      "  [PASS] Correctly detected missing package.json and index.js dynamically",
    );
    ...
    const hasMalformedSuggestion = dataMalformed.suggestions.some(
      (s) =>
        s.type === "configuration" &&
        s.description.includes("malformed or invalid JSON"),
    );
    if (!hasMalformedSuggestion)
      throw new Error(
        "Malformed package.json suggestion was not returned dynamically",
      );
    ...
    console.log(
      "Testing suggest with Notion scraper name but missing SDK dependency...",
    );
    fs.writeFileSync(
      path.join(suggestTempDir, "package.json"),
      JSON.stringify({
    ...
    const hasNotionSdkSuggestion = dataNotionSdk.suggestions.some(
      (s) =>
        s.type === "dependency" && s.description.includes("@notionhq/client"),
    );
    if (!hasNotionSdkSuggestion)
      throw new Error(
        "Missing notion client dependency suggestion not returned dynamically",
      );
    ...
    const hasMockSuggestion = dataMockCode.suggestions.some(
      (s) =>
        s.type === "implementation" &&
        s.description.includes("Dummy mock code detected"),
    );
    if (!hasMockSuggestion)
      throw new Error(
        "Mock code detection suggestion not returned dynamically",
      );
    console.log("  [PASS] Correctly detected mock code in index.js");

    // G. Exception handling block missing in index.js
    console.log(
      "Testing suggest with missing try-catch block in long index.js...",
    );
    fs.writeFileSync(
      path.join(suggestTempDir, "index.js"),
      `
  ...
  if (initialStatsRes.status !== 200)
    throw new Error("Failed to get initial stats");
  ...
  console.log(
    `  All 30 concurrent runs finished successfully. checking stats...`,
  );
