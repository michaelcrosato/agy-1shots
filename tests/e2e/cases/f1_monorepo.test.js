...
describe("F1: Monorepo Structure", () => {
  ...
  test("F1_4: Root contains README.md", () => {
    ...
    const jsonBlocks = [...content.matchAll(/```json\s*([\s\S]*?)\s*```/g)].map(
      (m) => m[1].trim(),
    );
    ...
    jsonBlocks.forEach((block) => {
      ...
      try {
        ...
        throw new Error(
          `Invalid JSON block in README.md: ${block}\nError: ${err.message}`,
        );
      }
    ...
    items.forEach((item) => {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        const pkg = JSON.parse(
          fs.readFileSync(path.join(fullPath, "package.json"), "utf8"),
        );
