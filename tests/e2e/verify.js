...
function writeManifestWithLock(manifestPath, manifest) {
  ...
  if (!hasLock) {
    throw new Error('Concurrency conflict: Could not acquire write lock on manifest.');
  }
...
const server = http.createServer((req, res) => {
              ...
              manifest: {
                ...
                latestFidelity:
                  evaluation && typeof evaluation.fidelityScore === 'number'
                    ...
                    : null,
                latestPassed:
                  evaluation && typeof evaluation.passed === 'boolean' ? evaluation.passed : null,
  ...
  if (url.pathname === '/api/manifest/spec') {
    ...
    parseJSONBody(req, (err, body) => {
      ...
      try {
        writeManifestWithLock(manifestPath, manifest);
  ...
  if (url.pathname === '/api/manifest/verify') {
    ...
    parseJSONBody(req, (err, body) => {
      ...
      if (
        attemptId &&
        (!Array.isArray(manifest.attempts) || !manifest.attempts.some((a) => a.id === attemptId))
      ) {
        sendJSON(404, { error: 'Attempt not found' });
        return;
      }
      ...
      if (cmd.includes('..') || (path.isAbsolute(cmd) && !cmd.startsWith(fullPath))) {
        sendJSON(400, {
          error: 'Security violation: command attempts to access paths outside target directory',
        });
        return;
      }
      ...
      timer = setTimeout(() => {
        ...
        if (process.platform === 'win32') {
          try {
            require('child_process').execSync(`taskkill /f /pid ${child.pid} /t`);
          } catch (e) {}
        } else {
          child.kill('SIGTERM');
        }
    ...
    parseJSONBody(req, (err, body) => {
      ...
      if (hasSecurityViolation) {
        ...
        sendJSON(200, {
          ...
          exitCode: 1,
          stdout: '',
          stderr: 'Security violation: command arguments attempt to escape target directory',
  ...
  if (url.pathname === '/api/export') {
    ...
    parseJSONBody(req, (err, body) => {
      ...
      const zipBytes = Buffer.from([
        0x50, 0x4b, 0x03, 0x04, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
async function runVerification() {
  ...
  if (!hasGenuineScraper) {
    ...
    fs.writeFileSync(path.join(scraperDir, 'package.json'), JSON.stringify(pkg, null, 2), 'utf8');
    fs.writeFileSync(
    ...
    console.log('Genuine notion-scraper directory found. Bypassing mock creation.');
  }
  ...
  runnerProcess.on('close', (code) => {
    ...
    server.close(() => {
      ...
      if (code === 0) {
        ...
        console.error('VERIFICATION FAILURE: Tests failed, exit code non-zero.');
        process.exit(1);
      }
    });
  });
