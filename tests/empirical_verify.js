...
async function runTests() {
  ...
  try {
    ...
    if (!pkgTags)
      throw new Error("Could not find temp-invalid-tags in scan results");
    if (!pkgName)
      throw new Error("Could not find temp-invalid-name in scan results");
    if (!pkgDesc)
      throw new Error("Could not find temp-invalid-desc in scan results");
    ...
    console.log(
      "  [PASS] Homepage returned status 200 OK (no crash on rendering)",
    );
  } finally {
    rmDirRecursive(invalidTagsDir);
    rmDirRecursive(invalidNameDir);
  ...
  try {
    ...
    if (!runData.error || !runData.error.includes("timeout")) {
      throw new Error(
        `Expected error to contain "timeout", got: ${runData.error}`,
      );
    }
    if (elapsed < 1400 || elapsed > 4000) {
      throw new Error(
        `Expected execution time to be close to 1.5s, got: ${elapsed}ms`,
      );
    }
    console.log(
      "  [PASS] Infinite loop correctly timed out and returned expected error structure",
    );
    ...
    if (process.platform === "win32") {
      try {
        ...
        if (procList.includes("loop.js")) {
          throw new Error(
            "Process loop.js is still running in the background!",
          );
        }
        console.log(
          "  [PASS] Process tree was successfully cleaned up (no zombie process)",
        );
      } catch (e) {
        if (e.message.includes("No Instance(s) Available")) {
          console.log(
            "  [PASS] Process tree was successfully cleaned up (no node processes)",
          );
        } else {
  ...
  if (polishRes.status !== 400) {
    throw new Error(
      `Expected polish script injection to return 400, got ${polishRes.status}`,
    );
  }
  ...
  console.log(
    "  [PASS] Modification of scripts key via /api/polish is blocked",
  );
  ...
  if (protoRes.status !== 400) {
    throw new Error(
      `Expected polish prototype pollution to return 400, got ${protoRes.status}`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      !runData.stderr.includes("Security violation")
    ) {
      throw new Error(
  ...
  try {
    ...
    if (envOutput.SAFE_VAR !== "hello-world") {
      throw new Error(
        `Expected SAFE_VAR to be hello-world, got: ${envOutput.SAFE_VAR}`,
      );
    }
    if (
      envOutput.NODE_OPTIONS &&
      envOutput.NODE_OPTIONS.includes("--max-old-space-size=4096")
    ) {
      throw new Error(
    ...
    console.log(
      "  [PASS] Dangerous environment keys (like NODE_OPTIONS) are stripped",
    );
  } finally {
    rmDirRecursive(envTestDir);
  }
