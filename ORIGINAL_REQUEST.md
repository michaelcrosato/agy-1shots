...
## Follow-up — 2026-06-18T05:53:04Z

Automate the gathering and recording of session metadata (model name, tool version, OS platform/build, execution time, and tokens) into the `oneshot.json` build attempts manifest via a Node.js CLI script.
...
### R1. Command-Line Interface (`scripts/record-attempt.js`)

Implement a Node.js CLI script that records a build attempt for a specified one-shot ID. It should support flags such as:

- `--id <one-shot-name>` (Required)
- `--model <name>` (Optional)
- `--tool <name>` (Optional)
- `--tool-build <version>` (Optional)
- `--build-tokens <count>` (Optional)
- `--build-time <ms>` (Optional)
- `--runtime-tokens <count>` (Optional)
- `--runtime-time <ms>` (Optional)
- `--session-log <path>` (Optional - path to a session JSON or text file containing token counts/duration)

### R2. Auto-Detection of Environment & Timestamp

The script must automatically capture host environment details:

- OS name (e.g., "Windows") and OS build version/release (e.g., "10.0.22631") using Node's `os` module.
- Current UTC ISO 8601 timestamp.
  It should write these into the `environment` and `timestamp` fields of the attempt.

### R3. Atomic Write and Schema Integrity

Read the existing `oneshot.json` file for the targeted one-shot. Ensure that:

- The `spec` block is never modified or deleted.
- Existing entries in the `attempts` array are never modified or removed.
- The new attempt entry is generated with a unique ID (matching the `att_<timestamp>_<random>` format or similar) and appended to the `attempts` array.
- The update is saved atomically to prevent partial writes.

### R4. Session Log Parsing

If `--session-log` is provided, the script should parse the log file:

- For JSON logs: look for keys such as `model`, `tokens`, `promptTokens`, `completionTokens`, `durationMs`, etc.
- For raw text logs: use regex patterns to detect statements like `tokens: 12345` or `1234ms` or `duration: 3500ms`.
- Allow CLI flags to override any values extracted from the log.

## Acceptance Criteria

### CLI Parameter Processing

- [ ] Running `node scripts/record-attempt.js --id <id> --model <model> --build-tokens 100 --build-time 50` successfully appends a valid attempt to the corresponding `oneshot.json`.
- [ ] Running the script with a non-existent one-shot ID exits with a non-zero code and logs a descriptive error.
- [ ] Running the script with invalid argument values (e.g., negative tokens or non-integer values) exits with a non-zero code and logs validation errors.

### Auto-Detection & Formatting

- [ ] The appended attempt has a unique string ID starting with `att_` (e.g., `att_1718690000000_abc123`).
- [ ] The appended attempt contains correct host environment info (`os` and `osBuild`) that matches the system platform and release.
- [ ] The appended attempt contains the correct ISO UTC timestamp for the time it was recorded.

### Session Log Parsing

- [ ] Providing a valid JSON log file correctly extracts model name, tokens, and duration, and records them in the attempt.
- [ ] Providing a raw text log file containing typical console outputs (e.g., "elapsed time: 1040ms", "tokens used: 154000") correctly extracts the values and records them.

### Integrity & Safety

- [ ] Existing entries in the `attempts` array are never modified or removed.
- [ ] The `spec` block (containing the permanent vision) is never changed.
- [ ] Saving updates is atomic and handles file access conflicts safely.

## Follow-up — 2026-06-18T13:40:31Z

Run the `agent-cleaner` suite on the target repository (`c:/dev/agy-1shots`) to bring it to a standard of excellence (zero lint/type/test errors, clean git state, matching docs, dependency hygiene), verified with programmatic and local gates on a safety branch `chore/agent-cleaner`.
...
### R1. safety and branch isolation

- Confirm the git repository is in a clean state (stash any uncommitted edits).
- Record the initial branch and HEAD commit hash.
- Create and switch to a new branch named `chore/agent-cleaner`.
- Do not force-push, hard-reset, or delete unmerged branches.

### R2. repository measurement & tooling configuration

- Analyze and log repository structure, file counts, LOC (lines of code), largest files, and manifests/lockfiles.
- Identify and run canonical gate commands (build, lint, test, format).
- Honor existing configuration and tools where present (e.g. ESLint/Prettier). If none exist, use standard defaults (Biome or Prettier+ESLint, tsc --noEmit).
- If additional linter/compiler rules are required, install them only as pinned devDependencies.

### R3. tiered cleanup plan

- Create a detailed cleanup plan in `.agent-cleaner/PLAN.md` divided into:
  1. Core configuration audit.
  2. Repository-wide formatting & import sorting.
  3. Per-module checks and fixes.
  4. Documentation & tests alignment.
  5. Integration validation.

### R4. execution & code hygiene

- Execute formatting, import sorting, and linting fixes. Fix lint errors to zero (or document as INTENTIONAL).
- Ensure TypeScript type checking passes without errors.
- Ensure all tests compile and pass, with no silent skips.
- Document and categorize every resolved/unresolved issue under one of three dispositions:
  - **FIXED**: Verified fix with command output evidence.
  - **NEEDS HUMAN DECISION**: Detailed context/options + recommendation.
  - **INTENTIONAL**: Documented and left as-is (e.g. legacy/third-party).
- Scan for committed secrets and ensure sensible `.gitignore` rules. Flag vulnerable dependencies.

### R5. verification & reporting

- Run all canonical build, lint, type, and test gates locally.
- Write a final state report to `.agent-cleaner/STATE.md` capturing:
  - Dispositions for each issue (FIXED / NEEDS DECISION / INTENTIONAL).
  - Pinned developer tools added.
  - Verification run commands and output logs.

## Acceptance Criteria

### Safety & Branching

- [ ] Active branch is `chore/agent-cleaner` branched from HEAD.
- [ ] No force-pushes or loss of unmerged branches.
- [ ] Working tree is clean at execution completion.

### Code Quality & Gates

- [ ] A plan file exists at `.agent-cleaner/PLAN.md`.
- [ ] The repository compiles/builds successfully without errors.
- [ ] Formatting is tool-enforced with zero formatting/linting errors.
- [ ] TypeScript type checks pass successfully (`tsc --noEmit` if JS/TS).
- [ ] All test suites run and pass.

### Documentation & Hygiene

- [ ] README quickstart setups and commands work literally without missing parameters.
- [ ] No committed secrets or excessive logs.
- [ ] A final report exists at `.agent-cleaner/STATE.md` listing the status of all cleaned components and gates execution evidence.
